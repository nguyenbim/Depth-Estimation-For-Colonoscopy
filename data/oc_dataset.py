...
def load_as_float(path, sequence_length):
    #rgb
    img = Image.open(path).resize((1024,512),Image.BICUBIC)
    stack = np.array(img).astype(np.float32)
...
class OC_Dataset(data.Dataset):
    ...
    def __init__(self, root, width=512, height=512, depth_transform=None):
        ...
        self.dataset = []
        import glob
        imgs = glob.glob(os.path.join(root, "*/*pair*"))
        print(imgs)
        #imgs.sort()
        #2900-3050
        """
    ...
    def __getitem__(self, index):
        ...
        #print("depth src: "+ str(depth_source_img.size()),"depth target: "+ str(depth_target_img.size()), "flow src: "+ str(flow_source_img.size()), "flow target: "+ str(flow_target_img.size) )
